...
// Zobrist Keys (Independent from Standard Engine)
let ZOBRIST_TABLE: number[][] = []; 
let SIDE_KEY: number = 0;

export const initZobristPro = () => {
...
class EnginePro {
    ...
    evaluate(): number {
        let score = 0;
        // Pro Engine Values: More aggressive, values mobility/attack slightly higher
        const M = [0, 10000, 220, 220, 420, 920, 460, 100]; 
        ...
        for(let i=0; i<BOARD_SIZE; i++) {
            if ((i&0x0F)<9 && (i>>4)<10) {
                ...
                if (p !== 0) {
                    ...
                    // Pro Evaluation Tuning
                    if (type === P_PAWN) {
                        const crossed = isRed ? r <= 4 : r >= 5;
                        if (crossed) {
                            val += 30; // More aggressive pawns
                            // Bonus for being close to King's file (col 3,4,5)
                            if (c >= 3 && c <= 5) val += 20;
                        }
                    } else if (type === P_HORSE) {
                        if (c === 4) val += 15; 
                        if (crossedRiver(r, isRed)) val += 30; // More bonus for attacking horse
    ...
    search(depth: number, alpha: number, beta: number, ply: number, isNull: boolean): number {
        ...
        // Null Move Pruning (Aggressive in Pro)
        ...
        for (let i = 0; i < moves.length; i++) {
            ...
            if (i === 0) {
                ...
                // Late Move Reduction (LMR) for Pro
                let reduction = 0;
                if (depth >= 3 && legalMoves > 4 && capture.captured === 0 && !isCheck) {
                    reduction = 1;
                }
...
const proEngine = new EnginePro();

export const resetEnginePro = () => {
    proEngine.reset();
};
...
export const getBestMove = async (
  ...
  lang: Language
    ...
    proEngine.loadBoard(board, turn);
    proEngine.abort = false;
    proEngine.nodes = 0;
    ...
    switch(difficulty) {
        case Difficulty.BEGINNER: maxDepth = 3; timeLimit = 800; break;
        case Difficulty.INTERMEDIATE: maxDepth = 5; timeLimit = 1500; break;
        case Difficulty.EXPERT: maxDepth = 7; timeLimit = 2500; break;
        case Difficulty.MASTER: maxDepth = 10; timeLimit = 4000; break;
        case Difficulty.GRANDMASTER: maxDepth = 24; timeLimit = 6000; break;
    }
    
    proEngine.stopTime = Date.now() + timeLimit;
    ...
    for (let d = 1; d <= maxDepth; d++) {
        let score = proEngine.search(d, alpha, beta, 0, false);
        
        // Aspiration Window Logic
        if (score <= alpha || score >= beta) {
            // Fell out of window, re-search with full width
            alpha = -30000;
            beta = 30000;
            score = proEngine.search(d, alpha, beta, 0, false);
        } else {
            // Tighten window for next iteration
            alpha = score - 50;
            beta = score + 50;
        }

        if (proEngine.abort) break;
        
        const hash = proEngine.getHash();
        const entry = proEngine.tt.get(hash);
        if (entry && entry.move) {
    ...
    if (bestMoveVal === 0) {
        const moves = proEngine.generateMoves();
        if (moves.length > 0) {
    ...
    const nodesK = (proEngine.nodes / 1000).toFixed(1) + 'k';
    const reasoning = lang === Language.CN 
        ? `[Pro] 深度 ${maxDepth} | 节点 ${nodesK} | 评分 ${scoreText} | 策略: 渴望窗口搜索`
        : `[Pro] Depth ${maxDepth} | Nodes ${nodesK} | Eval ${scoreText} | Strategy: Aspiration Search`;
